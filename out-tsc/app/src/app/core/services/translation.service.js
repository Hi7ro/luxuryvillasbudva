...
export class TranslationService {
    localeSignal = signal('de');
    locale = this.localeSignal.asReadonly();
    isGerman = computed(() => this.localeSignal() === 'de');
    setLocale(locale) {
    ...
    localizedPath(locale, path) {
        ...
        if (parts[0] && sections[parts[0]])
            parts[0] = sections[parts[0]];
        return `/${locale}${parts.length ? `/${parts.join('/')}` : ''}`;
    }
    static ɵfac = function TranslationService_Factory(__ngFactoryType__) { return new (__ngFactoryType__ || TranslationService)(); };
    static ɵprov = /*@__PURE__*/ i0.ɵɵdefineInjectable({ token: TranslationService, factory: TranslationService.ɵfac, providedIn: 'root' });
}
